...
step_size = '0.1';
step_half = false;
wwss = {'Inf','0.25','0.5','1','2','0.125'}; 
for wwid = 3%:length(wwss)
    ...
    load(['TotalAudioVisualData',lower(window_size),appendix,'.mat']);
 
    % loop through each test speaker
    for speaker_id = 1:length(speakers)
        %leave speaker_id out



        ffnn = ['feature_files/featureFile_winSize',window_size,'leaveout', ...
            speakers{speaker_id},appendix,'.csv'];

        ffnn2 = ['feature_files/featureFile_winSize',window_size,'leaveout', ...
            speakers{speaker_id},appendix,'_test.csv'];
        
        % first read from mrmr
%         mrmrffnn = ['mrmr_files/mrmrFile_winSize',window_size,'leaveout', ...
%             speakers{speaker_id},'.csv_results.txt'];
        
        mrmrffnn = ['mrmr_files/mrmrFile_winSize','Inf','leaveout', ...
            speakers{speaker_id},'.csv_results.txt'];
        ...
        for sub_speaker = 1:length(speakers)
            if sub_speaker ~= speaker_id
                CollectData = [CollectData; AllData{sub_speaker}(:,feat_ind)];
                CollectLabel = [CollectLabel; AllLabels{sub_speaker}];
                utt_string = AllUtterances{sub_speaker}(2:end);
                CollectUtt = [CollectUtt,',',utt_string];
                CollectSPeaker = [CollectSPeaker; repmat(sub_speaker, size(AllData{sub_speaker},1),1)];
            end
        end
        ...
        TestCollectData = [AllData{sub_speaker}(:,feat_ind)];
        TestCollectLabel = [AllLabels{sub_speaker}];
        Testutt_string = AllUtterances{sub_speaker}(2:end);
        TestCollectUtt = [',',Testutt_string];
        TestCollectSPeaker = [ repmat(sub_speaker, size(AllData{sub_speaker},1),1)];
        ...
        old_title = [newTitle,',',visual_title];
