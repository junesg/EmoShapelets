function [pred, ac, decv] = overpredictBotGPU(test_speaker,y, x, model,aiter)
...
for i=1:labelSetSize
  ...
  libsvmwrite([num2str(test_speaker),'/tempTestSVMDataFile',num2str(aiter),'.csv'], newClassLab, sparse(x));
  svmoutputFile = [' ',num2str(test_speaker),'/tempTestSVMDataOutput',num2str(aiter),'.txt'];
  %system( ['touch ',svmoutputFile]);
  system( [svmDir,'svm-predict  -q  -b 1'   , [' ',num2str(test_speaker),'/tempTestSVMDataFile',num2str(aiter),'.csv '], models{i}, svmoutputFile]);
