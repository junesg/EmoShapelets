function [model] = overtrainBotGPU(test_speaker,y, x,cmd, aiter)
...
for i=1:labelSetSize
    ...
    % print data to file
    libsvmwrite([num2str(test_speaker),'/tempSVMDataFile',num2str(aiter),'.csv'], newClassLab, sparse(x));
    

    system([svmDir,'svm-train-gpu ', [cmd,'  -w1 ', num2str(N)],  ...
        [' ',num2str(test_speaker),'/tempSVMDataFile',num2str(aiter),'.csv '],[svmModels,'class',num2str(i),'.txt '] ]);
     disp('finish writing')
     toc
