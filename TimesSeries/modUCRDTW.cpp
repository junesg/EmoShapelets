...
#define verbose 0 //wehther to print more infomration 
using namespace std;
...
/************************************************************************************************
**********************modified to be incorporated into processing data ***************************
********************************************************************************************
***********************/


/// Main Function
/* Input: m: size of query
 *        R: warping window
 *		  qp: query serience
 *		  fp: data series
 *
*/
struct stats FindClosesDTW(  int m, float R, vector<double> qp,  vector<double> fp, bool normalize)
{
    // FILE *fp;            /// data file pointer
    // FILE *qp;            /// query file pointer
    double bsf;          /// best-so-far
    double *t, *q, *nearest_tz;       /// data array and query array
...
// 	if (verbose)
//     	cout<<"BEFORE STARTING, r is "<< r<<" and R is "<< R <<" and m is "<<m <<endl;
// 
    ...
    if (verbose)
        cout<< "Size of index is : "<<sizeof(Index)<<endl;
    ...
    //Now fill in ex, ex2 and q based on length m
   
        // cout<<"======="<<"Printing qp: ";
        for (int itt = 0; itt< m; itt++){
        	double val = qp.at(itt);
            // cout<<val<<",";
        	ex += val;
        	ex2 += val*val;
        	q[itt] = val;
    ...
    // if (verbose)
        // cout<<"mean is "<<mean<<" and std is "<<std<<endl;

    if(normalize){
        // cout<< " printting the normalized"<<endl;
	    for( i = 0 ; i < m ; i++ ) {
	         q[i] = (q[i] - mean)/std;
             // cout<< q[i] << ",";
         }
    ...
    for( i = 0; i<m; i++)
    {
        ...
        Q_tmp[i].index = i;
        if(verbose){
            cout<<"before sorting: "<< i<< ": "<<Q_tmp[i].value<<endl;
        }
    }
    ...
    if(verbose) {
        for( i = 0; i<m; i++)
        {
            cout<<" at "<<i<<" the value is "<< Q_tmp[i].value<<endl;
        }
    }
    ...
    {   int o = Q_tmp[i].index;
        if (verbose)
            cout<<"The index  = "<<o << " at position "<<i<<endl;
        order[i] = o;
        qo[i] = q[o];
        uo[i] = u[o];
        lo[i] = l[o];
    ...
    if(verbose)
        cout<<"Freeing Q_temp"<<endl;

    free(Q_tmp);

    if(verbose)
        cout<<"AFTER Freeing Q_temp"<<endl;
    ...
    while(!done)
    {
        ...
        {   lower_upper_lemire(buffer, ep, r, l_buff, u_buff);
            ...
            if (it%(1000000/(EPOCH-m+1))==0 & verbose)
                fprintf(stderr,".");
                    ...
                    if (lb_kim < bsf)
                    {
                        ...
                        if (lb_k < bsf)
                        {
                            ...
                            if (lb_k2 < bsf)
                            {
                                ...
                                /*
                                 *	NEED TO WRITE A MULTIDIMENSIONAL DTW
                                 */
    ...
    if(verbose)
        cout<<"Finish finding Dist 1"<<endl;


    struct stats finalStat;
    finalStat.tz.erase(finalStat.tz.begin(), finalStat.tz.end());
    for (int tzint=0; tzint<m; tzint++){
        finalStat.tz.push_back(nearest_tz[tzint]);
    }
    // fclose(fp);
    if (verbose)
        cout<<"Finish finding Dist 2"<<endl;
    ...
    free(buffer);
    if (verbose)
        cout<<"Finish finding Dist 3"<<endl;
